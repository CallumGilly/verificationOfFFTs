...
module FFT.Leveled.FFT (cplx : Cplx) (M : Mon) where
  ...
  fft : (dft : {s : S l} → Ar (ι s) ℂ → Ar (ι s) ℂ)
       → (twid : ∀ {s p : S (ss l)} → P s → P p → ℂ)
       → {s : S (ss l)} → Ar s ℂ → Ar (transp s) ℂ
  fft dft twid {ι s} a = dft a
  fft dft twid {s ⊗ p} a = 
    let 
  ...
  fft-cong : {dft : ∀ {s : S l} → Ar (ι s) ℂ → Ar (ι s) ℂ}
              {twid : ∀ {s p : S (ss l)} → P s → P p → ℂ}
            → (dft-cong : ∀ {s} a b → (∀ i → a i ≡ b i) 
                        → ∀ i → dft {s} a i ≡ dft b i)
            → ∀ {s : S (ss l)} (a b : Ar s ℂ) → (∀ i → a i ≡ b i)
            → ∀ i → fft dft twid a i ≡ fft dft twid b i
  fft-cong {l} {dft} {twid} dft-cong {ι s} = dft-cong 
  fft-cong {l} {dft} {twid} dft-cong {s ⊗ s₁} a b a≡b (i ⊗ j) =
    fft-cong dft-cong _ _ 
      (λ k₁ → 
        cong (_ *_) (fft-cong dft-cong _ _ (λ k₂ → a≡b (k₂ ⊗ k₁)) j)
      ) 
      i

  fft-dft-cong : ∀ (dft₁ dft₂ : ∀ {s : S l} → Ar (ι s) ℂ → Ar (ι s) ℂ)
               → ∀ {twid : ∀ {s p} → P s → P p → ℂ}
               → (dft₂-cong : ∀ {n} a b → (∀ i → a i ≡ b i) 
                            → ∀ i → dft₂ {n} a i ≡ dft₂ b i)
               → (prf : ∀ {s : S l} → ∀ (ys : Ar (ι s) ℂ) → ∀ j → dft₁ ys j ≡ dft₂ ys j)
               → ∀ {s : S (ss l)}
               → ∀ (xs : Ar s ℂ)
               → ∀ i
               → fft dft₁ twid xs i ≡ fft dft₂ twid xs i
  fft-dft-cong dft₁ dft₂ dft₂-cong prf {ι s} = prf 
