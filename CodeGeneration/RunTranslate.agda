...
module _ where
  ...
  open import Matrix.Simple.Reshape as Simpᵣ using () 
  open import Data.Product
  ...
  oldFFT : ∀ {s : S (ss zz)} → Ar s ℂ → Ar s ℂ
  oldFFT {s} xs j = 
    let xs′ = Ar-from xs in
    let fftResult = Ar-to {ℂ} {S-from s} (Simpᵣ.reshape (Simpᵣ.♯ Simpᵣ.∙ (Simpᵣ.reindex ( sym (Simpᵣ.|s|≡|sᵗ| {S-from s .proj₁}) )) Simpᵣ.∙ Simpᵣ.♭) (FFT xs′)) in
    fftResult (j ⟨ subst (λ σ → Reshape σ s) (sym (S-inverse₁ s)) eq ⟩ )
...
main = run do
  ...
  --let dftAsVec = ArToVector $ oldFFT (reshape flatten-zᵣ input) 
  --let dftAsVec = ArToVector $ fftn input 
  --let fftAsVec = ArToVector $ reshape CMᵗ (fft dft twiddles (reshape flatten-zᵣ input))
  let dftAsVec = ArToVector $ fftn input
